...
def create_graph(center_lat, center_lon):
    ...
    threshold_distance = 1  # can change the threshold
...
def create_lockdown_graph(center_lat, center_lon):
    seconds_range = 100*2
    degree_range = seconds_range / 3600.0
    ...
    for lat in latitudes:
        for lon in longitudes:
            G_lockdown.add_node(node_counter, pos=(lat, lon), name=f"Node {node_counter}")
            node_counter += 1

    # add edges based on proximity
    threshold_distance = 1  # can change the threshold
    for node1 in G_lockdown.nodes:
        for node2 in G_lockdown.nodes:
            if node1 != node2:
                pos1 = np.array(G_lockdown.nodes[node1]['pos'])
                pos2 = np.array(G_lockdown.nodes[node2]['pos'])
                distance = np.linalg.norm(pos1 - pos2)
                if distance < threshold_distance:
                    G_lockdown.add_edge(node1, node2)

    return G_lockdown


def create_social_distancing_graph(center_lat, center_lon):
